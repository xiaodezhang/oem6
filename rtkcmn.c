                  ...
                  const double *R, int n, int m)
{
    ...
    for(i = 0;i < m;i++){
        printf("\n");
        for(j = 0;j < k;j++) printf("%f  ",H_[j+i*k]);
    }
#endif
    ...
    for(i = 0;i < m;i++){
        printf("\n");
        for(j = 0;j < m;j++) printf("%f  ",R[i+j*m]);
    }
#endif
    info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_);
#if 0
    printf("\nx:"); for(i = 0;i < k;i++) printf("%f  ",i < 3 ? xp_[i]-x_[i] : xp_[i]-xp_[5]);
#endif
#if 0
    printf("\nPp_:");
    for(i = 0;i < k;i++) printf("%f  ",Pp_[i+i*k]);
#endif
#if 0
    printf("\nv:");
    for(i = 0;i < m;i++) printf("%f  ",v[i]);
#endif
    for (i=0;i<k;i++) {
        x[ix[i]]=xp_[i];
