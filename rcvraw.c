...
extern int init_raw(raw_t *raw)
{
    ...
    raw->sta.hgt=0.0;
    raw->board = board0;
    raw->bestpos = bestpos0; raw->satvis = satvis0;
    raw->gpgsa = gpgsa0;
    raw->direc = direc0;
    raw->satpro = satpro0;
    return 1;
...
extern int init_hcnraw(hcnraw_t *hcnraw){
    ...
    hcnraw->msgtype[0] = '\0';
    hcnraw->time = time0;
    hcnraw->nbyte = hcnraw->len = 0;
    hcnraw->outtype = 0;
    hcnraw->board = board0;
    hcnraw->bestpos = bestpos0;
    hcnraw->satvis = satvis0;
    hcnraw->gpgsa = gpgsa0;
    hcnraw->direc = direc0;
#if 0
    hcnraw->satpro = satpro0;
    hcnraw->pipes = pipes0;
    printf("pvt:%d solstacode:%d velcode:%d satnum:%d\n",hcnraw->satpro.pvtstatus,
            hcnraw->satpro.solstacode,hcnraw->satpro.velcode,hcnraw->satpro.satnum);
#endif

#if 0
    for(i = 0;i < MAXPIPE;i++)
        if(!(hcnraw->pipes.pipe[i] = (pipe_t*)malloc(sizeof(pipe_t)))){
            exit(-1);
        }
...
extern void free_hcnraw(hcnraw_t *hcnraw){
    
#if 0
    int i;

    for(i = 0;i < MAXPIPE;i++)
        free(hcnraw->pipes.pipe[i]);
#endif
}
